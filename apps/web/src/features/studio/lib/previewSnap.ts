/** OpenCut-style preview snap (center-origin canvas coordinates). */
...
function pickClosestScaleCandidate<T extends { distance: number; edge: ScaleEdge }>({
  ...
  return candidates.reduce((bestCandidate, candidate) => {
    ...
    return shouldPreferCandidate && !shouldPreferBestCandidate ? candidate : bestCandidate;
  });
}
...
export function snapScale({
  ...
  const verticalTargets = [
    ...
    { position: centerX, line: { type: 'vertical' as const, position: centerX } },
  ...
  const horizontalTargets = [
    ...
    { position: centerY, line: { type: 'horizontal' as const, position: centerY } },
    { position: bottom, line: { type: 'horizontal' as const, position: bottom } },
  ];
...
export function snapScaleAxes({
  ...
  const currentAabbHalfW =
    (baseWidth * proposedScaleX * cosR + baseHeight * proposedScaleY * sinR) / 2;
  const currentAabbHalfH =
    (baseWidth * proposedScaleX * sinR + baseHeight * proposedScaleY * cosR) / 2;
  ...
  if (sinR > EPSILON) {
    for (const T of [canvasTop, 0, canvasBottom]) {
      ...
      if (distBottom <= snapThreshold.y) {
        ...
        if (Math.abs(scale) > MIN_SCALE) {
          xCandidates.push({ scale, distance: distBottom, line, edge: 'bottom' });
        }
      }
    }
  }
  ...
  if (cosR > EPSILON) {
    for (const T of [canvasTop, 0, canvasBottom]) {
      ...
      if (distBottom <= snapThreshold.y) {
        ...
        if (Math.abs(scale) > MIN_SCALE) {
          yCandidates.push({ scale, distance: distBottom, line, edge: 'bottom' });
        }
      }
    }
  }
...
export function snapRotation({
  ...
  const nearestRotationSnap =
    Math.round(proposedRotation / ROTATION_SNAP_STEP_DEGREES) * ROTATION_SNAP_STEP_DEGREES;
