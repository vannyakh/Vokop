...
export function useTimelineTracks(): TimelineTrackModel[] {
  ...
  return useMemo(() => {
    ...
    const labelOf = (id: string, fallback: string) =>
      timelineTrackLabels[id]?.trim() || fallback;
    ...
    const clipsForVisualTrack = (trackId: string, kinds: Array<'logo' | 'image' | 'sticker'>) =>
      canvasVisual
        .filter((c) => {
          ...
          // Legacy overlay placements show on the core image track.
          if (
            trackId === 'image' &&
    ...
    const tracks: TimelineTrackModel[] = [
      ...
      },
      {
        ...
        clips: [
          ...textClips,
          ...trackedTextClips.filter((c) => c.trackId === 'text'),
        ].sort((a, b) => a.start - b.start),
      },
      {
        id: 'image',
    ...
    for (const extra of extraTimelineTracks) {
      ...
      if (extra.type === 'text') {
        clips = trackedTextClips
          .filter((c) => c.trackId === extra.id)
          .sort((a, b) => a.start - b.start);
      ...
      // sound / audio extras stay empty until multi-audio clips are supported
