/**
 * Dependency-free color helpers (adapted from OpenCut `utils/color.ts`,
 * with the culori dependency replaced by standard RGB/HSV/HSL math).
 *
 * Hex values are handled WITHOUT a leading `#` (6 or 8 chars), matching the
 * OpenCut color-picker API.
 */
...
/** Parse a hex string (3/4/6/8 chars, no `#`) to normalized RGB. */
...
/** Parse `rgb()/rgba()` and `hsl()/hsla()` functional notation. */
function parseCssFunctionColor(input: string): RgbColor | null {
  ...
  return { ...hslToRgb(h, Math.max(0, Math.min(1, s)), Math.max(0, Math.min(1, l))), alpha };
}
