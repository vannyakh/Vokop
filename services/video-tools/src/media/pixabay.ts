...
import { getPixabayKey, hasPixabayKey, mediaCacheTtl } from './config.js';
...
export async function searchPixabayImages(
  ...
  const cacheKey = `vokop:media:pixabay:images:${query}:${page}:${perPage}`;
...
export async function searchPixabayVideos(
  ...
  const cacheKey = `vokop:media:pixabay:videos:${query}:${page}:${perPage}`;
