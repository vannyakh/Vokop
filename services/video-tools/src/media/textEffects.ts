...
import { hasPixabayKey, mediaCacheTtl } from './config.js';
...
export async function getTextEffectPreviews(): Promise<{
  previews: TextEffectPreview[];
  ...
  const previews = await Promise.all(TEXT_EFFECT_SEEDS.map((seed) => previewForSeed(seed)));
  const payload = { previews, pixabayEnabled };

  if (pixabayEnabled && previews.some((p) => p.previewURL)) {
    try {
      const redis = getRedis();
      await redis.setEx(CACHE_KEY, mediaCacheTtl(), JSON.stringify(payload));
    } catch {
